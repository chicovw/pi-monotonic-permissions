...
const expected = [
  'package.json', 'README.md', 'LICENSE', 'SECURITY.md', 'CHANGELOG.md', 'CONTRIBUTING.md', 'policy.example.json',
  ...['index', 'config', 'requests', 'paths', 'policy', 'bash', 'inspection'].map(n => `src/${n}.ts`),
  ...['architecture', 'threat-model', 'policy-reference', 'evaluation', 'claims-review'].map(n => `docs/${n}.md`)
].sort();
